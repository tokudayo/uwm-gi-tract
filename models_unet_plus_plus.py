...
class Generic_UNetPlusPlus(SegmentationNetwork):
    ...
    def forward(self, x):
        ...
        x0_3 = self.loc2[2](
            torch.cat([x0_0, x0_1, x0_2, self.up2[2](x1_2)], 1))
        ...
        x1_3 = self.loc1[2](
            torch.cat([x1_0, x1_1, x1_2, self.up1[2](x2_2)], 1))
        x0_4 = self.loc1[3](
            torch.cat([x0_0, x0_1, x0_2, x0_3, self.up1[3](x1_3)], 1))
        ...
        x2_3 = self.loc0[2](
            torch.cat([x2_0, x2_1, x2_2, self.up0[2](x3_2)], 1))
        x1_4 = self.loc0[3](
            torch.cat([x1_0, x1_1, x1_2, x1_3, self.up0[3](x2_3)], 1))
        x0_5 = self.loc0[4](
            torch.cat([x0_0, x0_1, x0_2, x0_3, x0_4, self.up0[4](x1_4)], 1))
    ...
    def create_nest(self, z, num_pool, final_num_features, num_conv_per_stage, basic_block, transpconv):
        ...
        for u in range(z, num_pool):
            ...
            if not self.convolutional_upsampling:
                tu.append(Upsample(
                    scale_factor=self.pool_op_kernel_sizes[-(u + 1)], mode=self.upsample_mode))
            else:
...
def build_model(CFG):
    model = Generic_UNetPlusPlus(
        input_channels=3,
        base_num_features=20,
        num_classes=CFG.num_classes,
        num_pool=CFG.num_pool,
    )
    model.to(CFG.device)
    return model
