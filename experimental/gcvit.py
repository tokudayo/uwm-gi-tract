...
class GCViT(nn.Module):
    def __init__(self,
        ...
        for i in range(len(depths)):
            ...
            setattr(self, 'norm_{}'.format(i), nn.LayerNorm(int(dim * 2 ** i)))
    ...
    def forward_features(self, x):
        ...
        for i, level in enumerate(self.levels):
            feat, x = level(x, return_features=True)
            output_features.append(getattr(self, f"norm_{i}")(feat).permute(0, 3, 1, 2))

        # output_features[-1] = self.norm(output_features[-1])
